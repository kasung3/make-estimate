...
/* harmony import */ var _home_ubuntu_make_estimate_nextjs_space_app_api_admin_coupons_route_ts__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./app/api/admin/coupons/route.ts */ "(rsc)/./app/api/admin/coupons/route.ts");
...
const routeModule = new next_dist_server_future_route_modules_app_route_module_compiled__WEBPACK_IMPORTED_MODULE_0__.AppRouteRouteModule({
    ...
    resolvedPagePath: "/home/ubuntu/make_estimate/nextjs_space/app/api/admin/coupons/route.ts",
    nextConfigOutput,
    userland: _home_ubuntu_make_estimate_nextjs_space_app_api_admin_coupons_route_ts__WEBPACK_IMPORTED_MODULE_3__
});
// Pull out the exports that we need to expose from the module. This should
// be eliminated when we've moved the other routes to the new format. These
// are used to hook into the route.
const { requestAsyncStorage, staticGenerationAsyncStorage, serverHooks } = routeModule;
const originalPathname = "/api/admin/coupons/route";
function patchFetch() {
...
async function GET() {
    try {
        ...
        const coupons = await _lib_db__WEBPACK_IMPORTED_MODULE_3__.prisma.platformCoupon.findMany({
            orderBy: {
                createdAt: "desc"
            }
        });
...
async function POST(request) {
    try {
        ...
        const { code, type, trialDays, allowedPlans, maxRedemptions, expiresAt } = body;
        if (!code || !type) {
            return next_server__WEBPACK_IMPORTED_MODULE_0__.NextResponse.json({
        ...
        const existing = await _lib_db__WEBPACK_IMPORTED_MODULE_3__.prisma.platformCoupon.findUnique({
            where: {
                code: normalizedCode
            }
        });
        ...
        if (type === "free_forever") {
            ...
            const stripeCoupon = await _lib_stripe__WEBPACK_IMPORTED_MODULE_5__.stripe.coupons.create({
                percent_off: 100,
                duration: "forever",
            ...
            const promoCode = await _lib_stripe__WEBPACK_IMPORTED_MODULE_5__.stripe.promotionCodes.create(promoCodeParams);
            stripePromoCodeId = promoCode.id;
        }
        const coupon = await _lib_db__WEBPACK_IMPORTED_MODULE_3__.prisma.platformCoupon.create({
            data: {
                code: normalizedCode,
                type,
