...
/* harmony import */ var _home_ubuntu_make_estimate_nextjs_space_app_api_admin_plans_route_ts__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./app/api/admin/plans/route.ts */ "(rsc)/./app/api/admin/plans/route.ts");
...
const routeModule = new next_dist_server_future_route_modules_app_route_module_compiled__WEBPACK_IMPORTED_MODULE_0__.AppRouteRouteModule({
    ...
    resolvedPagePath: "/home/ubuntu/make_estimate/nextjs_space/app/api/admin/plans/route.ts",
    nextConfigOutput,
    userland: _home_ubuntu_make_estimate_nextjs_space_app_api_admin_plans_route_ts__WEBPACK_IMPORTED_MODULE_3__
});
...
/***/ "(rsc)/./app/api/admin/plans/route.ts":
/*!**************************************!*\
...
async function GET() {
    try {
        ...
        const plans = await _lib_db__WEBPACK_IMPORTED_MODULE_3__.prisma.billingPlan.findMany({
            orderBy: {
                sortOrder: "asc"
            },
            include: {
        ...
        return next_server__WEBPACK_IMPORTED_MODULE_0__.NextResponse.json(plans);
    } catch (error) {
...
async function ensureStripePrice(productId, amountCents, interval, planKey, seatModel) {
    // Search for existing price with same amount and interval
    const existingPrices = await _lib_stripe__WEBPACK_IMPORTED_MODULE_5__.stripe.prices.list({
        product: productId,
        active: true,
        limit: 100
    });
    const matchingPrice = existingPrices.data.find((p)=>p.unit_amount === amountCents && p.recurring?.interval === interval && p.currency === "usd");
    if (matchingPrice) {
    ...
    const newPrice = await _lib_stripe__WEBPACK_IMPORTED_MODULE_5__.stripe.prices.create(priceData);
...
async function POST(request) {
    try {
        ...
        const { planKey, name, priceMonthlyUsdCents, priceAnnualUsdCents, seatModel, boqLimitPerPeriod, boqTemplatesLimit, coverTemplatesLimit, logoUploadAllowed, sharingAllowed, maxActiveMembers, features, isMostPopular, sortOrder, active } = body;
        ...
        const existing = await _lib_db__WEBPACK_IMPORTED_MODULE_3__.prisma.billingPlan.findUnique({
            where: {
                planKey
            }
        });
        ...
        const resolvedSeatModel = seatModel || "single";
        // Create Stripe product
        const stripeProduct = await _lib_stripe__WEBPACK_IMPORTED_MODULE_5__.stripe.products.create({
        ...
        const monthlyPriceId = await ensureStripePrice(stripeProduct.id, priceMonthlyUsdCents, "month", planKey, resolvedSeatModel);
        // Create annual Stripe price if provided
        let annualPriceId = null;
        if (priceAnnualUsdCents) {
            annualPriceId = await ensureStripePrice(stripeProduct.id, priceAnnualUsdCents, "year", planKey, resolvedSeatModel);
        }
        // Create plan in database
        const plan = await _lib_db__WEBPACK_IMPORTED_MODULE_3__.prisma.billingPlan.create({
            data: {
                planKey,
                name,
        ...
        await _lib_db__WEBPACK_IMPORTED_MODULE_3__.prisma.billingPlanPriceHistory.create({
            data: {
        ...
        return next_server__WEBPACK_IMPORTED_MODULE_0__.NextResponse.json(plan);
    } catch (error) {
