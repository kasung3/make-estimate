// ISO 3166-1 country list with phone codes
// Top common countries are listed first for UX
...
// Commonly used countries (displayed first in dropdowns)
...
// Full alphabetical list of all countries
...
// Get unique countries - common ones first, then alphabetical (avoiding duplicates)
export function getCountries(): Country[] {
  const commonCodes = new Set(COMMON_COUNTRIES.map(c => c.code));
  const otherCountries = ALL_COUNTRIES.filter(c => !commonCodes.has(c.code));
  return [...COMMON_COUNTRIES, ...otherCountries];
}
...
// Export all countries for lookup
